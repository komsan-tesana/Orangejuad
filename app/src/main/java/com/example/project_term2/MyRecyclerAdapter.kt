...
class MyRecyclerAdapter(fragmentActivity: FragmentActivity, val dataSource: JSONArray) :  RecyclerView.Adapter<MyRecyclerAdapter.Holder>() {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        holder.layout.setOnClickListener{
            ...
            val benefit:String = dataSource.getJSONObject(position).getString("benefit").toString()
            val science:String = dataSource.getJSONObject(position).getString("science").toString()



            val detail = detail().newInstance(title,image,description,benefit,science)
            val transaction : FragmentTransaction = fm.beginTransaction()
            transaction.replace(com.example.project_term2.R.id.layout,detail,"fragment_detail")
